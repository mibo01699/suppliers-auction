...
| التطبيق | الوظيفة | التكامل مع BIGISH-YER |
|---|---|---|
| **AJYAL** | إدارة المستفيدين والأكواد | عبر نظام المقاصة (`clearing-system.js`) |
| **GAV** | نقاط البيع وصرف السلع | عبر واجهات برمجة التطبيقات (APIs) `/api/pos/*` |
| **BIGISH-YER** | العمود الفقري المالي (YER) | يوفر واجهات برمجة تطبيقات (APIs) `/api/yer/*` |
| **suppliers-auction** | منصة المزادات | يستخدم `/api/yer/transfer` لتنفيذ الدفع الهجين |

**الآن أصبحت جميع التطبيقات الأربعة متكاملة ومترابطة ضمن منظومة واحدة تعتمد على `BIGISH-YER` كمركز مالي مشترك.** 🚀
